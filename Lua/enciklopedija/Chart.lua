...
function barChart( frame )
    ...
    function tooltip( gi, i, val )
        ...
        if suffix == '(x legend)' then
            suffix = '(' .. (xlegends[i] or xlegends[1] or '') .. ')'
        elseif suffix == '(x year)' then
            local n = tonumber(xlegends[i] or xlegends[1] or '0' )
            if not n then
                suffix = '(' .. (xlegends[i] or xlegends[1] or '') .. ')'
            else
                suffix = '(' .. (xlegends[i] or xlegends[1] or '') .. ' m.)'
            end
        end
        return mw.ustring.gsub(groupName, '_', ' ') .. prefix .. ' ' ..
            mw.ustring.gsub(mw.ustring.gsub(mw.language.new('en'):formatNum( tonumber( val ) or 0 ), ',', ' '), '%.', ',') .. ' ' .. suffix, false
    end
